...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *big.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if evm.Config.NoRecursion && evm.depth > 0 {
		if evm.firehoseContext.Enabled() {
			evm.firehoseContext.EndFailedCall(gas, true, "NoRecursion")
		}
	...
	if evm.Context.ExtraValidator != nil && evm.depth > 0 {
		if evm.Context.ExtraValidator.IsAddressDenied(caller.Address(), common.CheckFrom) ||
			...
			if evm.firehoseContext.Enabled() {
				evm.firehoseContext.EndFailedCall(gas, true, types.ErrAddressDenied.Error())
			}
...
func (evm *EVM) create(caller ContractRef, codeAndHash *codeAndHash, gas uint64, value *big.Int, address common.Address, typ OpCode) ([]byte, common.Address, uint64, error) {
	...
	if evm.Context.CanCreate != nil {
		if !evm.Context.CanCreate(evm.StateDB, caller.Address(), evm.Context.BlockNumber) {
			if evm.firehoseContext.Enabled() {
				evm.firehoseContext.EndFailedCall(gas, true, ErrUnauthorizedDeveloper.Error())
			}
