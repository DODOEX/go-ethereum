...
func TestTrieForkGC(t *testing.T) {
	...
	db := rawdb.NewMemoryDatabase()
	genesis := (&Genesis{BaseFee: big.NewInt(params.InitialBaseFee)}).MustCommit(db)
	blocks, _ := GenerateChain(params.TestChainConfig, genesis, engine, db, 2*128, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{1}) })
...
func TestLargeReorgTrieGC(t *testing.T) {
	...
	original, _ := GenerateChain(params.TestChainConfig, shared[len(shared)-1], engine, db, 2*128, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{2}) })
	competitor, _ := GenerateChain(params.TestChainConfig, shared[len(shared)-1], engine, db, 2*128+1, func(i int, b *BlockGen) { b.SetCoinbase(common.Address{3}) })
	...
	for i, block := range competitor[:len(competitor)-128] {
		if node, _ := chain.stateCache.TrieDB().Node(block.Root()); node != nil {
...
func testSideImport(t *testing.T, numCanonBlocksInSidechain, blocksBetweenCommonAncestorAndPruneblock int) {
	...
	blocks, _ := GenerateChain(params.TestChainConfig, genesis, engine, db, 2*128, nil)
	diskdb := rawdb.NewMemoryDatabase()
	(&Genesis{BaseFee: big.NewInt(params.InitialBaseFee)}).MustCommit(diskdb)
	chain, err := NewBlockChain(diskdb, nil, params.TestChainConfig, engine, vm.Config{}, nil, nil)
	if err != nil {
	...
	lastPrunedIndex := len(blocks) - 128 - 1
	lastPrunedBlock := blocks[lastPrunedIndex]
	firstNonPrunedBlock := blocks[len(blocks)-128]
	...
	fork, _ := GenerateChain(params.TestChainConfig, parent, engine, db, 2*128, func(i int, b *BlockGen) {
		b.SetCoinbase(common.Address{2})
	})
